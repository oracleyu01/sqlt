...
NOT IN 연산자를 사용한 서브쿼리는 ANTI JOIN으로 변환될 수 있으며, 메인 쿼리의 데이터에서 서브쿼리에 없는 데이터를 찾는 방식입니다. 성능 최적화를 위해 HASH ANTI JOIN을 사용하는 것이 효율적입니다.
...
🤔 문제: DALLAS에서 근무하지 않는 사원들의 이름과 월급 출력하세요
...
🤔 문제: 작은 테이블(DEPT)을 해시 테이블로 설정하여 튜닝하세요.
...
・ IN 연산자에는 hash_sj 힌트를, NOT IN 연산자에는 hash_aj 힌트를 사용하여 성능을 최적화할 수 있습니다.
・ swap_join_inputs를 사용하면 서브쿼리의 작은 테이블을 해시 테이블로 설정하여 성능을 향상시킬 수 있습니다.
・ NULL 처리와 실행 계획 확인은 튜닝 과정에서 중요한 단계입니다.
