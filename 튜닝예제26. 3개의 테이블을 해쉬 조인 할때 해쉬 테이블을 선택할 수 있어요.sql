...
HASH 조인을 사용할 때, 작은 테이블을 메모리에 올려 해시 테이블로 구성하는 것이 성능 최적화의 핵심입니다. 2개의 테이블 조인 시에는 leading 힌트를 사용하여 해시 테이블을 지정할 수 있지만, 3개 이상의 테이블 조인에서는 추가적인 힌트(swap_join_inputs, no_swap_join_inputs)를 활용해야 합니다.
...
・ 3개 이상의 테이블을 HASH 조인할 때, 적절한 힌트와 조인 순서를 설정하여 성능을 최적화할 수 있습니다.
・ 작은 테이블 또는 조건으로 데이터 접근량이 적은 테이블을 메모리에 올리는 것이 중요합니다.
・ swap_join_inputs와 no_swap_join_inputs 힌트를 활용해 해시 테이블과 탐색 테이블을 명확히 설정할 수 있습니다.
