...
데이터 분석 함수는 기존의 스칼라 서브쿼리나 복잡한 SQL 문을 간소화하고, 
실행 성능을 극대화할 수 있는 강력한 도구입니다. 
특히, OVER 절을 사용하여 누적 합계, 순위 등을 효율적으로 계산할 수 있습니다.

🎓 결론

・ 스칼라 서브쿼리는 반복 실행으로 인해 성능 저하를 유발할 수 있으므로 
   데이터 분석 함수로 대체하는 것이 효과적입니다.
...
-- https://github.com/oracleyu01/sqlt/blob/main/튜닝예제44. 데이터 분석함수를 이용해서 SQL을 재작성할 줄 알아야해요. 네번째.sql
